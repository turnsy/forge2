...
- [x] Implement `lib/uploads/parse-upload.ts`:
  - Accept `File` / `Buffer` + mime/extension
  - Return discriminated union: `{ kind: 'csv' | 'pdf' | 'xlsx', name, text, meta }` or `{ ok: false, code, message }`
- [x] **CSV:** keep as CSV string; optional row/sample truncation with footer noting truncation
- [x] **PDF:** extract text; chunk with simple headings (`## Page N` or detected headings); Markdown-friendly plain text
- [x] **XLSX:** list sheet names in `meta.sheets`; export selected sheet(s) as CSV-like text
- [x] Implement `lib/uploads/select-xlsx-sheets.ts`:
  - If one sheet → use it
...
- [x] Implement `lib/uploads/normalize-message-uploads.ts` for batch of files + prompt text
- [x] Implement `lib/uploads/context-storage.ts`:
...
- [x] Add `POST /api/coach/upload-context` with `requireRole('coach')`
- [x] Unit tests: CSV sample, multi-sheet XLSX, PDF fixture (small), over-limit rejection, unsupported extension
- [x] Map errors to stable codes: `FILE_TOO_LARGE`, `TOO_MANY_FILES`, `UNSUPPORTED_TYPE`, `PARSE_FAILED`, `XLSX_NEEDS_SHEET`
...
- [ ] Create Supabase Storage bucket (or path policy) for ephemeral `draft-uploads/` — RLS scoped to coach (migration: `supabase/migrations/20260602120000_draft_uploads_storage.sql`)
...
- [x] Given fixtures, `normalizeMessageUploads()` returns expected text shapes
- [x] 6th file or oversize file fails with clear error code
- [x] Multi-sheet XLSX without sheet hint returns clarification payload (not thrown exception)
- [x] Normalized text persisted to Storage; **not** written to `plans` / `plan_versions`
- [x] No normalized upload files mounted or copied into Vercel Sandbox
