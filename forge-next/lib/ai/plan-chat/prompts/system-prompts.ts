export function buildCoachAgentSystemPrompt(input: {
  ...
  const sections = [
    ...
    "- Mutations: accept_coach_link, reject_coach_link, assign_plan, unassign_plan, mark_assignment_complete",
    "- Open a saved plan for editing (not already in preview): set_current_artifact({ type: \"plan\", id }) — e.g. \"edit Summer Block\", \"add a week to this plan\"",
    "- Open an athlete's in-progress assignment for editing: set_current_artifact({ type: \"assignment\", id: athleteId }) — e.g. \"add a deload week to Jane's current plan\"",
    ...
    "- Create or change the in-preview plan: get_plan_codegen_guide, then submit_plan_code (include assignmentId when editing an assigned plan)",
    "",
    "When editing an assigned plan (loaded via set_current_artifact with type=\"assignment\"):",
    "- Sets with status \"completed\" or \"skipped\" are LOCKED",
    "- You may only add new sets/exercises/days/weeks (planned), or modify existing planned sets",
    "- Do not remove exercises, days, or weeks that contain any locked sets",
    "- The forge_plan library will raise errors if you try to modify locked content",
    "- Preview updates after each sandbox run — nothing is saved until the coach clicks Save",
    "",
