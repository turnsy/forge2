...
export function cloneSetFromPrevious(previous: Set): Set {
...
export function EditableExerciseBlock({
  exercise,
  exerciseIndex,
  exerciseCount,
  disabled,
  ...
  function handleRemoveSet(setIndex: number) {
    ...
    onExerciseChange({
      ...exercise,
      sets: exercise.sets.filter((_, index) => index !== setIndex) as typeof exercise.sets,
    });
  }
  ...
  return (
    <section
      className={[
        compact ? "space-y-3" : accordionNestedClass(),
        compact ? "" : "space-y-3 p-4",
      ].join(" ")}
      data-exercise-editable="true"
    >
