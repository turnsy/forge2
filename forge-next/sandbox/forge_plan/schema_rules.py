...
def validation_rules_cheat_sheet() -> str:
    ...
    return "\n".join(
        [
            ...
            "- add_exercise: appends a standalone exercise block (type: exercise) with empty sets",
            "- create_exercise(name, sets=[]) -> exercise dict; create_superset(e1, e2, ..., notes=None) -> superset block dict",
            "- add_superset(week_index, day_index, block): append a composed superset block",
            "- add_superset(week_index, day_index, notes=None) -> SupersetRef: empty superset; add exercises with SupersetRef.add_exercise(name)",
            "- SupersetRef.add_exercise(name): name only, empty sets — same as Plan.add_exercise for standalone blocks",
            "- For supersets, add sets with Plan.add_set(..., exercise_name=...) — same as standalone exercises",
